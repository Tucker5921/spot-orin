...
class SpotFetchROS2Node(Node):
    def __init__(self):
        ...
        # bounding box 尺寸限制
        self.max_bbox_x = 500.0   # bbox 寬度超過這個值就不處理
        self.max_bbox_y = 450.0   # bbox 高度超過這個值就不處理
        ...
        self.image_pubs = {
            'frontleft_fisheye_image': self.create_publisher(
                Image,
                'image/frontleft',
                10
            ),
            'frontright_fisheye_image': self.create_publisher(
                Image,
                'image/frontright',
                10
            ),
    ...
    def get_obj_and_img(self, image_sources, nearest_target):
        ...
        for source in image_sources:
            ...
            for obj in resp.object_in_image:
                ...
                # bbox 過大就不處理這個 obj
                if not self.is_valid_polygon_bbox(obj.image_properties.coordinates):
                    continue
    ...
    # ---------------------------------------------------------        
    def is_valid_polygon_bbox(self, polygon):
        min_x = math.inf
        min_y = math.inf
        max_x = -math.inf
        max_y = -math.inf
        ...
        if bbox_width > self.max_bbox_x:
            self.get_logger().warn(
                f"bbox 寬度過大，跳過: width={bbox_width:.1f}, limit={self.max_bbox_x:.1f}"
            )
            return False

        if bbox_height > self.max_bbox_y:
            self.get_logger().warn(
                f"bbox 高度過大，跳過: height={bbox_height:.1f}, limit={self.max_bbox_y:.1f}"
            )
            return False

        return True
    def image_response_to_cv2(self, image_response):
        """
        將 Spot SDK 的 image_response 轉成 OpenCV image。
        注意：這裡不旋轉，避免 bbox 座標對不上。
        """
        ...
        if image.format == image_pb2.Image.FORMAT_RAW:
            if image.pixel_format == image_pb2.Image.PIXEL_FORMAT_RGB_U8:
                img = img.reshape((image.rows, image.cols, 3))
                img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)

            elif image.pixel_format == image_pb2.Image.PIXEL_FORMAT_RGBA_U8:
                img = img.reshape((image.rows, image.cols, 4))
                img = cv2.cvtColor(img, cv2.COLOR_RGBA2BGR)

            elif image.pixel_format in [
                ...
                self.get_logger().warn("未知 RAW image pixel format")
                return None
    ...
    def detections_on_image(self, cv_image, objects):
        """
        將 NCS 偵測結果畫到 OpenCV image 上。
        objects = resp.object_in_image
        """
        if cv_image is None:
            return None
        ...
        for obj in objects:
            ...
            polygon = obj.image_properties.coordinates

            pts = []
            for vert in polygon.vertexes:
                pts.append([int(vert.x), int(vert.y)])
            ...
            # 算中心點
            center_x, center_y = self.find_center_px(polygon)
    ...
    def publish_image(self, source_name, image_response, objects):
        """
        將模型結果畫在影像上，旋轉校準後發布給 RViz。
        """
        cv_image = self.image_response_to_cv2(image_response)

        if cv_image is None:
            self.get_logger().warn("無法轉換 image_response 成 OpenCV image")
            return

        # 先畫 bbox / 中心點
        image = self.detections_on_image(cv_image, objects)

        if image is None:
            return

        # 再旋轉整張已經畫好的 debug image
        image = self.rotate_image(image, source_name)

        msg = self.bridge.cv2_to_imgmsg(image, encoding='bgr8')
        msg.header.stamp = self.get_clock().now().to_msg()
        msg.header.frame_id = source_name

        if source_name not in self.image_pubs:
            self.get_logger().warn(f"沒有對應的 image publisher: {source_name}")
            return

        self.image_pubs[source_name].publish(msg)

    def rotate_image(self, cv_image, source_name):
        """
        根據相機來源旋轉整張 image。
        """
        if cv_image is None:
        ...
        if source_name not in ROTATION_ANGLE:
            return cv_image

        angle = ROTATION_ANGLE[source_name]
