...
class SpotFetchROS2Node(Node):
    ...
    def fetch_loop(self):
        ...
        if abs(angle_to_target) > 1.2:
            ...
            return
        # 6. 若最近目標還大於 1.3m，就接近最近目標
        elif nearest_distance > 1.3:
    ...
    def get_result_callback(self, future):
        ...
        if status == 4:
            ...
            self.get_logger().error('夾取被中止 (ABORTED)')
            # if target is not None:
            #     target["status"] = STATUS_PENDING # 讓它有機會被 fetch_loop 重新選中
        else:
            self.get_logger().error(f'夾取已取消，狀態碼: {status}')
