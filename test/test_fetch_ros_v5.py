    ...
    def manip_feedback_callback(self, feedback_msg):
        ...
        if state_code != self.last_feedback_state:
            ...
            if self.state_start_time is not None:
                ...
                if (elapsed > self.state_timeout_duration and state_code != 3) or (elapsed > self.state_timeout_duration*2 and state_code == 3):
                    self.get_logger().error(f"❌ 夾取狀態卡在 {state_code} 超時，判定失敗。")
                    self.state_start_time = now
